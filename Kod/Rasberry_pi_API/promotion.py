...
def order_files_left_preferred(file_idx_list: List[int], color: str) -> List[int]:
    ...
    """
    return sorted(file_idx_list, reverse=(color == "white"))
...
def promotion_move(msg: dict) -> List[Step]:
    ...
    to = msg["to"]
    fen = msg["fen"]
    piece_removed = msg.get("piece_removed", "pawn")
    piece_placed = msg.get("piece_placed", "piece")
    color = msg.get("color", "white")
    dir = 1 if color == "white" else -1

    tx, ty = square_to_coords(to)
    removed_park_x, removed_park_y = compute_capture_parking_from_fen(
        piece_removed, color, fen
    )

    fx, fy = square_to_coords(frm)
    placed_park_x, placed_park_y = compute_capture_parking_from_fen(
    ...
    steps = [
        Step(
            ...
            placed_park_y,
            placed_park_x,
            placed_park_y + LANE_OFFSET * dir,
        ),
        Step(
            "H slide one",
            placed_park_x,
            placed_park_y + LANE_OFFSET * dir,
            removed_park_x + LANE_OFFSET * dir,
            placed_park_y + LANE_OFFSET * dir,
        ),
        Step(
            "V slide to parking",
