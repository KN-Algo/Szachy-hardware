...
def parking_mm_for_pawn_from_fen(color_captured: str, fen: str) -> Tuple[float, float]:
    ...
    x_mm = (x_file + 0.5) * SQUARE_SIZE
    y_mm = (y_rank + 0.5) * SQUARE_SIZE
    return x_mm, y_mm
...
def capture_move(msg: dict) -> List[Step]:
    ...
    s1 = Step(
        ...
        t_x=park_x,
        t_y=park_y,
        note=f"Remove captured {piece_captured} ({color_captured}) {to} -> parking",
    )

    # 2) przenosimy bijącą figurę z FROM na TO
